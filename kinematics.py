...
def getkin(x: float, y: float, z: float, rx: float, ry: float, rz: float):
    rx = np.radians(rx)
    ry = np.radians(ry)
    rz = np.radians(rz)

    desired_pos = np.matrix([[np.cos(ry)*np.cos(rz), np.cos(ry)*np.sin(rz), -np.sin(ry), x],
                             [np.sin(rx)*np.sin(ry)*np.cos(rz)-np.cos(rx)*np.sin(rz), np.sin(rx)*np.sin(
                                 ry)*np.sin(rz)+np.cos(rx)*np.cos(rz), np.sin(rx)*np.cos(ry), y],
                             [np.cos(rx)*np.sin(ry)*np.cos(rz)+np.sin(rx)*np.sin(rz), np.cos(rx)*np.sin(
                                 ry)*np.sin(rz)-np.sin(rx)*np.cos(rz), np.cos(rx)*np.cos(ry), z],
                             [0, 0, 0, 1]])
...
if __name__ == "__main__":
    print(getkin(1.5708, -1.5708, 1.5708, 0, 1.5708, -3.1416))
